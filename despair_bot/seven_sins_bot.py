...
async def setup(ctx):
    ...
    if total_new == 0:
        ...
        embed.description = (
            ...
            f"**{len(created_categories)}** channel category/ies, and "
            f"**{len(created_channels)}** channel(s).\n\n"
            "**Role categories created:** `╔══ SINNERS ══╗` · `╔══ VIRTUES ══╗` · "
            "`╔══ MYTHS ══╗` · `╔══ DESPAIR ══╗` · `╔══ HOPE ══╗`\n"
            "Drag the relevant roles beneath each header in **Server Settings → Roles** "
            "to group them visually in the member list.\n\n"
            "Run `!setup` again any time — it skips anything that already exists."
        )
